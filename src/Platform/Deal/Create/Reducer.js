...
const intialState = {
  ...
  formData: {
    ...
    sponsored: {
      ...
      selectedOption: {value: 'sponsored', label: 'sponsored'},
      rules: ['required'],
      validation: '',
    },
    private_mode: {
...
export default (state = intialState, action) => {
  switch(action.type) {
    case 'DEAL_CREATE_UNMOUNT':
      return intialState;
    case 'DEAL_CREATE_SET_STATUS':
      return fromJS(state).set('status', action.status).toJS()
    case 'DEAL_CREATE_ON_SELECT':
      return fromJS(state)
        ...
        .toJS();
    case 'DEAL_CREATE_INPUT_VALIDATION':
      return fromJS(state)
        .setIn(action.path, action.validation)
        .toJS()
    case 'DEAL_CREATE_DATE_UPDATE':
      return fromJS(state)
        .setIn(action.path.concat(['value']), action.date)
        .toJS()
    case 'DEAL_CREATE_ON_CHANGE':
      return fromJS(state)

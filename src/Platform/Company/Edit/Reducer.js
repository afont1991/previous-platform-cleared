...
export default (state = intialState, action) => {
  switch(action.type) {
    case 'COMPANY_EDIT_UNMOUNT':
      return intialState;
    case 'COMPANY_EDIT_REQUEST_PROFILE':
      return Utils.MapProfileToState(action.response.body.payload, state)
    case 'COMPANY_EDIT_SET_ID':
      return fromJS(state).setIn(['formData', 'submit_data', 'CompanyId'], action.id).toJS();
    case "COMPANY_EDIT_SET_STATUS":
    ...
    case "COMPANY_EDIT_INIT_FORM":
      ...
      Object.keys(lookups).forEach((lookupKey) => {
        ...
        if(lookupKey === 'TypesOfCapital'){
          ...
          state.formData.lender_transactions.TypesOfCapital.selectedOption = {}
